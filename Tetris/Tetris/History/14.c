	...
	for(int i = 0; i < 4; i++){
		for(int j = 0; j < 4; j++){
			if((fig.model)[i][j]){ // && (staticlevel[fig.y + i][fig.x + j]
				if((fig.x + i > 19) | (fig.y + j < 0) | (fig.x + i < 0)) 
					return 1;
				else if(staticlevel[fig.y + i][fig.x + j])
					return 1;
			}
		}
	}
...
void movRotate(char staticlevel[20][10], struct figure *fig){
	int yPos_B = (*fig).y + (*fig).bottompoint;
	int yPos = (*fig).y;
	int xPos = (*fig).x;
	...
	(*fig).bottompoint = minBottom((*fig).model);
	(*fig).toppoint = maxTop((*fig).model);
	if((isCollision(staticlevel, *fig)) | ((*fig).x + (*fig).leftpoint < 0) | ((*fig).x + (*fig).rightpoint > 9)){
		if((((*fig).height) == 3) | ((*fig).width == 3))
			rotateArray_ACW((*fig).model, (*fig).width, (*fig).height, (*fig).leftpoint, (3 - (*fig).height) );
		else if((((*fig).height) == 4) | ((*fig).width == 4))
			rotateArray_ACW((*fig).model, (*fig).width, (*fig).height, 0, 0);
		(*fig).x = xPos;
		(*fig).y = yPos;
		(*fig).rightpoint = maxRight((*fig).model);
		(*fig).leftpoint = minLeft((*fig).model);
		...
		(*fig).x = xPos_L - (*fig).leftpoint;
		(*fig).y = yPos_B - (*fig).bottompoint;
	}	
}
...
void rotateArray_CW(char arr[4][4], int width, int height, int startPoint_x, int startPoint_y){
	...
	for (int i=(startPoint_y);i<n/2 + 1;i++) 
  { 
