...
int main(void)
{
	...
	showScore(screen, maxRight(fig_L), ScorePoint_x, ScorePoint_y);
	ScreenUpdate(screen);
...
void movRotate(char staticlevel[20][10], struct figure *fig){
	char temp;
	int xPos_L = (*fig).x + (*fig).leftpoint;
	int xPos_R = (*fig).x + (*fig).rightpoint;
	for(int i = 0; i < 4; i++){
		for(int j = 0; j < 4; j++){
			temp = (*fig).model[i][j];
	...
	for(int i = 0; i < 4; i++){
		for(int j = 0; j < 2; j++){
			temp = (*fig).model[i][j];
			(*fig).model[i][j] = (*fig).model[i][4 - j - 1];
			(*fig).model[i][4 - j - 1] = temp;
		}
	}
	(*fig).rightpoint = maxRight((*fig).model);
	(*fig).leftpoint = minLeft((*fig).model);
